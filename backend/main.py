...
from key_management import (
    ...
    update_clarifier_fatigue,
    update_last_request_types,
    get_silent_misalignment_rate
)
...
class APOResponse(BaseModel):
    ...
    iterations: int
    critic_score: float
    critic_comments: List[Dict[str, Any]] = Field(default_factory=list)
    clarifier_options: Optional[List[str]] = None
    pathfinder_options: Optional[List[str]] = None
    continuation_available: Optional[bool] = None
    continuation_message: Optional[str] = None
    suggested_action: Optional[str] = None
    quality_score_best: Optional[float] = None
    ...
    rephrase_similarity: Optional[float] = None
    rejection_hypotheses: Optional[List[str]] = None
    intent_type: Optional[str] = None
    risk_tier: Optional[str] = None
    clarifier_fatigue_active: Optional[bool] = None
    silent_misalignment: Optional[bool] = None
    minimalism_signal: Optional[bool] = None
    critic_rubric_used: Optional[str] = None
...
async def generate_prompt_by_key(
    ...
    try:
        ...
        silent_misalignment_rate = await get_silent_misalignment_rate(str(user.get("client_id")), db_pool)
        clarifier_count_last_5 = int(user.get("clarifier_count_last_5") or 0)
        ...
        results = await apo_workflow(
            ...
            clarifier_fatigue_count=clarifier_count_last_5,
            silent_misalignment_rate=silent_misalignment_rate
        )
        ...
        try:
            await increment_usage_count(
                str(user.get("client_id")),
                db_pool,
                tokens_est,
                cost_est,
                results.get("silent_misalignment", False)
            )
            await update_last_request_types(str(user.get("client_id")), db_pool, results.get("intent_type"))
            if results.get("output_type") == "clarifier":
                await update_clarifier_fatigue(str(user.get("client_id")), db_pool, 1)
            else:
                await update_clarifier_fatigue(str(user.get("client_id")), db_pool, -1)
        except Exception:
            pass
...
async def generate_optimized_content(
    ...
    try:
        ...
        silent_misalignment_rate = await get_silent_misalignment_rate(str(current_user.get("client_id")), db_pool)
        clarifier_count_last_5 = int(current_user.get("clarifier_count_last_5") or 0)
        ...
        try:
            ...
            await update_last_request_types(str(current_user.get("client_id")), db_pool, results.get("intent_type"))
            if results.get("output_type") == "clarifier":
                await update_clarifier_fatigue(str(current_user.get("client_id")), db_pool, 1)
            else:
                await update_clarifier_fatigue(str(current_user.get("client_id")), db_pool, -1)
        except Exception:
            pass
